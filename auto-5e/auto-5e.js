...
Hooks.on('preDeleteCombat', (entity, options) => {
    DistributeExperience(entity);
});

Hooks.on('updateToken', (entity, options) => {
    SyncTokenStatus(entity);
});

Hooks.on('updateCombat', entity => {
    PanOnNextTurn(entity);
});
...
// Pans players to the token who's turn it is in combat. Also selects the token if they own it.
// This will eventually get configs so panning can be turned off for players.
function PanOnNextTurn(entity) {
    if (!game.combat || game.combat.turns.length === 0) {
        return;
    }
    token = entity.turns[entity.current.turn].token;
    let xCoord = token.x;
    let yCoord = token.y;
    canvas.animatePan({
        x: xCoord,
